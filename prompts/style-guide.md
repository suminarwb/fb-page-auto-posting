> **⚠️ DRAFT AWAL — BELUM DIREVIEW**
> File ini dibuat otomatis sebagai starting point berdasarkan `docs/01-PRD.md` §8 (Content & Brand Guidelines). Ini **bukan** brand voice final — tolong dibaca, direvisi, atau ditulis ulang oleh Suminar sebelum dipakai untuk publish sungguhan ke Facebook Page Stik Satu. Nada bicara ("gue/lo" vs "aku/kamu"), contoh caption, dan daftar yang boleh/tidak boleh di bawah ini semuanya masih tebakan awal, bukan hasil observasi konten existing Stik Satu.

# Style Guide — Brand Voice Stik Satu

## Siapa Stik Satu

Stik Satu adalah Facebook Page komunitas gaming untuk audiens Indonesia. Fokus utama **console gaming** (PlayStation, Xbox, Nintendo), PC gaming jadi topik sekunder/selingan. Isi bukan berita atau review formal — ini kanal cerita personal soal pengalaman main game, kenangan, dan opini santai dari sudut pandang orang yang genuinely suka gaming, bukan media/press.

## Bahasa & Nada

- Bahasa Indonesia casual, gaya ngobrol, bukan bahasa baku/formal.
- Sapaan default: **"gue/lo"** — *(DRAFT: sesuaikan kalau tone existing Stik Satu ternyata pakai "aku/kamu" atau campuran, cek histori post lama kalau ada)*.
- Boleh selipan istilah gaming umum (grinding, farming, boss fight, backlog, plot twist, dsb) tapi jangan berlebihan sampai kedengaran sok gaul atau kayak marketing copy.
- Hindari bahasa press release / clickbait ("WAJIB TAHU!", "GAK NYANGKA!", ALL CAPS berlebihan).
- Emoji boleh dipakai secukupnya untuk penekanan, bukan di setiap kalimat.

## Format Konten

- **Storytelling personal**, bukan review teknis atau berita rilis. Sudut pandang: "gue inget waktu...", "ada momen di game X yang bikin gue...", bukan "Game X adalah judul yang dirilis pada..."
- Panjang caption: idealnya 3-8 kalimat pendek (kira-kira 400-900 karakter). Cukup untuk satu momen/cerita, tidak perlu esai panjang.
- Satu post = satu momen/ide, bukan rangkuman banyak topik sekaligus.
- Penutup yang mengundang interaksi itu bagus (pertanyaan santai ke pembaca), tapi tidak wajib di setiap post — jangan jadi formula kaku yang keliatan robotic.

## Hashtag

- **Wajib** ada di setiap caption, ditaruh di baris terakhir (terpisah dari isi cerita), supaya jangkauan postingan lebih luas.
- **2-4 hashtag per post** — relevan ke topik spesifik yang dibahas (nama game/franchise, platform seperti #PlayStation/#Xbox/#Nintendo), plus tag komunitas **#StikSatu**.
- Jangan asal banyak/generic (`#gaming #game #viral #fyp` bertumpuk) — pilih yang benar-benar nyambung ke cerita, bukan spam supaya kelihatan "SEO".
- Hashtag tidak dihitung sebagai bagian dari storytelling-nya — ini elemen distribusi, bukan isi cerita, jadi tidak perlu terasa "menyatu" secara naratif dengan paragraf di atasnya.

## Fokus Konten

- **Utama:** console gaming — PS/Xbox/Nintendo, first-party maupun third-party title, kenangan generasi konsol lama juga oke.
- **Sekunder:** PC gaming — boleh diangkat sesekali, tapi jangan sampai porsinya menyaingi konten console.
- Hindari topik yang murni soal hardware/teknis (benchmark, spek, harga part) — itu bukan gaya storytelling personal Stik Satu.

## Yang Harus Dihindari

- Klaim yang menyesatkan atau tidak bisa diverifikasi (rumor rilis, bocoran belum resmi yang diklaim pasti benar, angka statistik yang dikarang).
- Konten sensitif: SARA, politik praktis, kekerasan eksplisit, konten dewasa.
- Merendahkan platform/komunitas lain (perang console war yang toxic) — bercanda ringan soal rivalitas platform boleh, tapi tidak merendahkan.
- Promosi produk/judi/investasi yang tidak relevan dengan gaming.
- Nada keluhan/negatif berlebihan yang tidak konstruktif (post boleh cerita momen frustrasi di game, tapi bukan rant kebencian).

## Contoh Caption (ilustrasi tone, bukan konten final)

> *(DRAFT — contoh ini karangan untuk ilustrasi format, ganti/hapus kalau tidak representatif)*
>
> "Masih inget banget malem pertama kali tamatin final boss di [judul game], jam udah nunjukin lewat tengah malem tapi rasanya sayang banget kalau berhenti pas udah sedeket itu. Begadang emang, tapi worth it — itu salah satu momen paling puas yang gue rasain dari sebuah game tahun ini. Ada yang pernah ngerasain momen kayak gini juga?"

## Catatan Revisi

Bagian yang paling butuh input Suminar langsung:
1. Konfirmasi sapaan "gue/lo" vs alternatif lain.
2. Contoh caption real dari post Stik Satu sebelumnya (kalau ada) untuk dijadikan referensi tone yang lebih akurat.
3. Daftar topik/game spesifik yang jadi fokus saat ini (rilis terbaru yang relevan, franchise favorit audiens).
